...
def airport_map_rows(origin: str, dest: str, airports: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for code, label in [(origin, "Origin"), (dest, "Destination")]:
        ...
        rows.append({"code": code, "label": label, "lat": lat, "lon": lon, "name": airport_display_name(code, airports)})
...
destination_arrival_text, destination_arrival_source = estimate_destination_arrival_text(
    selected_date,
    selected_time,
    float(feature_row["Distance"].iloc[0]),
    lookup_details,
)
...
with map_tab:
    try:
        ...
        map_metrics[0].metric("From", origin)
        map_metrics[1].metric("To", dest)
        map_metrics[2].metric("Land around", destination_arrival_text)
        st.pydeck_chart(
            pdk.Deck(
